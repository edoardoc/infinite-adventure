...
const programId = new PublicKey('ALF2mCoNcyjspCXoc6cdjAV2JcM63kguTqEMPo4YULg1'); // Replace with your program ID
...
async function main() {
  ...
  console.log('Game Data Account Address:', gameDataAccount.toBase58());
  console.log('Game Map Account Address:', gameMapAccount.toBase58());
  try {
  ...
  async function movePlayerLoop() {
    while (true) {
      const moveDirection = await new Promise<string>((resolve) => {
        rl.question('\nEnter direction to move (north, south, east, west) or collect or "stop" to exit: ', resolve);
      });
      ...
      if (moveDirection.toLowerCase() === 'collect') {

        console.log(`\nAttempting to collect...`);
        try {
          const tx = await program.methods
            .collectItem("rare mushroom")
            .accounts({
              gameDataAccount: gameDataAccount as PublicKey,
              gameMapAccount: gameMapAccount as PublicKey,
              authority: payer.publicKey,
            ...
            .signers([payer])
            .rpc();
          console.log(`collected! Transaction:`, tx);
